...
class BottleneckV2Unit(tf.keras.layers.Layer):
	...
	def get_config(self):
		...
		config.update({"num_filters": self._num_filters})
		config.update({"stride": self._stride})
		return config
...
class ResnetV2(tf.keras.Model):
	...
	def __init__(self,
		...
		for b, (f, u, s) in enumerate(zip(num_filters, num_units, strides), 1):
			n = "block{}".format(b)
			# print(f'-> Creating block: num_units={u}, num_filters={f}, stride={s}, name={n}')
	...
	def compute_output_shape(self, input_shape):
		...
		for i, layer in enumerate(self._pre_head):
	...
	def get_connected_model(self):
		x = self.model_input
		for layer in self._root.layers:
		...
		for i, block in enumerate(self._blocks):
			for j, layer in enumerate(block.layers):
				layer._inbound_nodes = []
				x = layer(x)
				layer._name = f'block{i+1}_unit{j+1}'
		for layer in self._pre_head:
			layer._inbound_nodes = []
			x = layer(x)
		if self._head is not None:
			x = self._head(x)
		return tf.keras.models.Model(inputs=self.model_input, outputs=x)
...
if __name__ == "__main__":
	...
	# Building the model and initializing weights
	...
	model = model.get_connected_model()
	print(model.summary())
	...
	new_model = tf.keras.models.Model(inputs=model.inputs, outputs=layer_outputs)
